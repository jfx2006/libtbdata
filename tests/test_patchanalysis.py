...
class PatchAnalysisTest(MockTestCase):
    ...
    def assertWarnings(self, warnings, expected_warnings):
        missed_warnings = [
            ...
            if ew not in [str(w.message) for w in warnings]
        ]
        ...
        unexpected_warnings = [
            str(w.message) for w in warnings if str(w.message) not in expected_warnings
        ]
    ...
    def test_bug_analysis(self):
        ...
        self.assertIn(
            str(exc.exception),
            [
                "Too many matching authors (jwalden+bmo@mit.edu, anarchy@gentoo.org) found for jwalden@mit.edu",
                "Too many matching authors (anarchy@gentoo.org, jwalden+bmo@mit.edu) found for jwalden@mit.edu",
            ],
        )

        info = patchanalysis.bug_analysis(
            846986, author_cache={"jwalden@mit.edu": ["jwalden+bmo@mit.edu"]}
        )
        ...
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["code_churn_last_3_releases"], 36
        )
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["developer_familiarity_overall"], 13
        )
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["developer_familiarity_last_3_releases"], 1
        )
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["reviewer_familiarity_overall"], 0
        )
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["reviewer_familiarity_last_3_releases"], 0
        )
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["languages"], ["License", "Makefile"]
        )
        ...
        self.assertEqual(
            info["patches"]["1584ba8c1b86"]["url"],
            "https://hg.mozilla.org/mozilla-central/rev/1584ba8c1b86",
        )
        self.assertEqual(
            info["landings"]["nightly"],
            datetime(2010, 4, 5, 23, 12, 52, tzinfo=pytz.UTC),
        )
        ...
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["code_churn_last_3_releases"], 31
        )
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["developer_familiarity_overall"], 4
        )
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["developer_familiarity_last_3_releases"], 4
        )
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["reviewer_familiarity_overall"], 16
        )
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["reviewer_familiarity_last_3_releases"], 0
        )
        self.assertEqual(len(info["patches"]), 1)
        self.assertEqual(info["patches"]["8641afbd20e6"]["source"], "mercurial")
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["url"],
            "https://hg.mozilla.org/mozilla-central/rev/8641afbd20e6",
        )
        self.assertEqual(
            info["patches"]["8641afbd20e6"]["languages"], ["C", "C++", "Makefile"]
        )
        self.assertEqual(
            info["landings"]["nightly"],
            datetime(2011, 5, 26, 6, 40, 11, tzinfo=pytz.UTC),
        )
        self.assertIsNone(info["landings"]["release"])
        self.assertIsNone(info["landings"]["beta"])
        self.assertIsNone(info["landings"]["aurora"])
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertEqual(
                info["patches"]["154b951082e3"]["code_churn_last_3_releases"], 245
            )
            self.assertEqual(
                info["patches"]["154b951082e3"]["developer_familiarity_overall"], 2
            )
            self.assertEqual(
                info["patches"]["154b951082e3"][
            ...
            self.assertEqual(
                info["patches"]["154b951082e3"]["url"],
                "https://hg.mozilla.org/mozilla-central/rev/154b951082e3",
            )
            self.assertEqual(
                info["landings"]["nightly"],
                datetime(2016, 5, 19, 16, 54, 30, tzinfo=pytz.UTC),
            )
            self.assertIsNone(info["landings"]["release"])
            self.assertIsNone(info["landings"]["beta"])
            self.assertIsNone(info["landings"]["aurora"])
        ...
        self.assertEqual(
            info["patches"]["b9d0984bdd95"]["code_churn_last_3_releases"], 62
        )
        self.assertEqual(
            info["patches"]["f5578fdc50ef"]["code_churn_last_3_releases"], 121
        )
        self.assertEqual(
            info["patches"]["b9d0984bdd95"]["developer_familiarity_overall"], 5
        )
        self.assertEqual(
            info["patches"]["f5578fdc50ef"]["developer_familiarity_overall"], 5
        )
        self.assertEqual(
            info["patches"]["b9d0984bdd95"]["developer_familiarity_last_3_releases"], 5
        )
        self.assertEqual(
            info["patches"]["f5578fdc50ef"]["developer_familiarity_last_3_releases"], 5
        )
        self.assertEqual(
            info["patches"]["b9d0984bdd95"]["reviewer_familiarity_overall"], 48
        )
        self.assertEqual(
            info["patches"]["f5578fdc50ef"]["reviewer_familiarity_overall"], 9
        )
        self.assertEqual(
            info["patches"]["b9d0984bdd95"]["reviewer_familiarity_last_3_releases"], 1
        )
        self.assertEqual(
            info["patches"]["f5578fdc50ef"]["reviewer_familiarity_last_3_releases"], 2
        )
        self.assertEqual(
            info["patches"]["b9d0984bdd95"]["languages"], ["C", "C++", "Windows IDL"]
        )
        self.assertEqual(
            info["patches"]["f5578fdc50ef"]["languages"], ["C", "Makefile", "Shell"]
        )
        self.assertEqual(len(info["patches"]), 2)
        self.assertEqual(info["patches"]["b9d0984bdd95"]["source"], "mercurial")
        self.assertIn(
            info["patches"]["b9d0984bdd95"]["url"],
            "https://hg.mozilla.org/mozilla-central/rev/b9d0984bdd95",
        )
        self.assertEqual(info["patches"]["f5578fdc50ef"]["source"], "mercurial")
        self.assertIn(
            info["patches"]["f5578fdc50ef"]["url"],
            "https://hg.mozilla.org/mozilla-central/rev/f5578fdc50ef",
        )
        self.assertEqual(
            info["landings"]["nightly"],
            datetime(2011, 12, 11, 4, 15, 3, tzinfo=pytz.UTC),
        )
        self.assertIsNone(info["landings"]["release"])
        self.assertIsNone(info["landings"]["beta"])
        self.assertIsNone(info["landings"]["aurora"])
        ...
        self.assertEqual(
            info["landings"]["nightly"],
            datetime(2016, 6, 10, 13, 42, 45, tzinfo=pytz.UTC),
        )
        self.assertIsNone(info["landings"]["release"])
        self.assertIsNone(info["landings"]["beta"])
        self.assertIsNone(info["landings"]["aurora"])
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertEqual(
                info["landings"]["nightly"],
                datetime(2016, 6, 6, 15, 4, 18, tzinfo=pytz.UTC),
            )
            self.assertIsNone(info["landings"]["release"])
            self.assertIsNone(info["landings"]["beta"])
            self.assertIsNone(info["landings"]["aurora"])
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertWarnings(
                w,
                [
                    "Reviewer jocheng@mozilla.com is not in the list of reviewers on Bugzilla ("
                    + ", ".join(sorted(["benjamin@smedbergs.us", "jaas@kflag.net"]))
                    + ").",
        ...
        self.assertEqual(
            info["landings"]["nightly"],
            datetime(2016, 3, 26, 2, 12, 27, tzinfo=pytz.UTC),
        )
        self.assertEqual(
            info["landings"]["aurora"],
            datetime(2016, 3, 28, 23, 24, 6, tzinfo=pytz.UTC),
        )
        self.assertEqual(
            info["landings"]["beta"], datetime(2016, 3, 31, 19, 48, 30, tzinfo=pytz.UTC)
        )
        self.assertEqual(
            info["landings"]["release"],
            datetime(2016, 4, 6, 19, 35, 18, tzinfo=pytz.UTC),
        )
        self.assertEqual(
            info["landings"]["esr"], datetime(2016, 4, 7, 18, 41, 3, tzinfo=pytz.UTC)
        )
        ...
        info = patchanalysis.bug_analysis(
            829557, author_cache={"dholbert@cs.stanford.edu": ["dholbert@mozilla.com"]}
        )
        ...
        info = patchanalysis.bug_analysis(
            1161913, reviewer_cache={"mt": "martin.thomson@gmail.com"}
        )
    ...
    def test_uplift_info(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertIsNotNone(info["uplift_reviewer"])
            self.assertEqual(
                info["uplift_reviewer"]["name"], "lukasblakk+bugs@gmail.com"
            )
    ...
    def test_patch_info(self):
        ...
        self.assertEqual(
            info["land"]["aurora"], utils.get_date_ymd("2011-07-07 19:58:31")
        )
        self.assertEqual(
            info["land"]["beta"], utils.get_date_ymd("2011-07-07 19:59:52")
        )
        self.assertEqual(
            info["land"]["release"], utils.get_date_ymd("2011-07-07 23:25:27")
        )
        self.assertEqual(
            info["land"]["nightly"], utils.get_date_ymd("2011-07-07 19:25:02")
        )
